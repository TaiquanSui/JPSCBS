...
void ThreadSafeInterruptor::interrupt() {
    interrupted.store(true);
    cv.notify_all();
}

bool ThreadSafeInterruptor::is_interrupted() const {
    return interrupted.load();
}

void ThreadSafeInterruptor::reset() {
...
void BenchmarkUtils::write_results_to_csv(const std::string& filename,
                                        const std::vector<BenchmarkResult>& results) {
    std::ofstream file(filename);
    if (!file.is_open()) {
        throw std::runtime_error("Unable to create CSV file: " + filename);
    }
...
std::vector<BenchmarkResult> BenchmarkUtils::run_scen_file(
    ...
    while (num_agents <= all_agents.size() && last_success) {
        ...
        try {
            ...
            // 改进：使用 std::async 运行 solver，防止阻塞主线程
            ...
            if (future.wait_for(std::chrono::duration<double>(time_limit)) == std::future_status::timeout) {
                ...
                try {
                    solution = future.get();  // If solver threw an exception, catch it
                } catch (const std::exception& e) {
                    logger::log_error("Solver error: " + std::string(e.what()));
                    timeout = true;
                    solution.clear();  // Ensure solution is empty
                }
            }
...
std::pair<std::vector<BenchmarkResult>, std::vector<BenchmarkResult>> 
BenchmarkUtils::run_algorithm_comparison(
    const std::string& map_file,
    const std::string& scen_file,
    ...
    logger::log_info("Starting CBS algorithm test...");
    auto cbs_results = run_scen_file(map_file, scen_file, cbs_solver, time_limit);
    
    std::this_thread::sleep_for(std::chrono::seconds(1));
    
    logger::log_info("Starting JPSCBS algorithm test...");
    auto jpscbs_results = run_scen_file(map_file, scen_file, jpscbs_solver, time_limit);
    
    return {cbs_results, jpscbs_results};
}

void BenchmarkUtils::run_all_scenarios(SolverFunction solver) {
    ...
    try {
                ...
                for (const ScenType& scenario : {
                    ...
                    for (int i = 1; i <= 1; ++i) {
                        ...
                        if (fs::exists(scen_file)) {
                            ...
                            auto results = run_scen_file(map_path, scen_file, solver);
        ...
        // 输出到CSV文件
        write_results_to_csv("benchmark_results.csv", all_results);
...
void BenchmarkUtils::run_all_scenarios_comparison(
    SolverFunction solver1, 
    SolverFunction solver2) {
    
    std::vector<BenchmarkResult> all_results1;
    std::vector<BenchmarkResult> all_results2;   
    ...
    try {
            ...
            if (map_entry.path().extension() == ".map") {
                ...
                for (const ScenType& scenario : {
                    ...
                    for (int i = 1; i <= 25; ++i) {
                        ...
                        if (fs::exists(scen_file)) {
                            logger::log_info("Testing scenario: " + scen_file);
                            auto [results1, results2] = run_algorithm_comparison(
                                map_path, scen_file, solver1, solver2);
                            all_results1.insert(all_results1.end(), 
                                            results1.begin(), 
                                            results1.end());
                            all_results2.insert(all_results2.end(), 
                                            results2.begin(), 
                                            results2.end());
                        }
                    }
        ...
        auto stats1 = calculate_stats(all_results1);
        stats1.print();
        
        logger::log_info("\nJPSCBS Algorithm Statistics:");
        auto stats2 = calculate_stats(all_results2);
        stats2.print();
